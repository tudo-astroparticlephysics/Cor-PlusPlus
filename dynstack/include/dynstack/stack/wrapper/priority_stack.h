...
namespace dynstack
{
    namespace wrapper
    {
        ...
        public:
            ...
            PriorityStack(const PriorityStack<TStack, TFunc>& rhs)
            : m_oStack_if(rhs.m_oStack_if ), m_oStack_else( rhs.m_oStack_else )
            { }
            ...
            inline int push_back(TType&& data)
            {
                ...
                if (decision)
                {
                    const int ret = m_oStack_if.push_back(data);
            ...
            inline int push_back(std::unique_ptr<const TType[]> data, const unsigned int elem)
            {
                ...
                for (unsigned int i = 0; i < elem; i++)
                {
                    ...
                    if (decision)
                    {
                        const int ret = m_oStack_if.push_back(data[i]);
                        check = check | ret;
                    }
                    else
                    {
                        const int ret = m_oStack_else.push_back(data[i]);
                        check = check | ret;
                    }
